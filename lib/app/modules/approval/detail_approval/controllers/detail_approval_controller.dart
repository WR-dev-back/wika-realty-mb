...
class DetailApprovalController extends GetxController {
  ...
  var approvalDetail = Rx<ApprovalDetail>(
    ApprovalDetail(
      status: false, // Set default status as false
      message: '',
      data: Data(
        approvalDetails: [],
        id: '', // Initialize with empty list or default value
      ),
    ),
  );
  ...
  void onInit() {
    super.onInit();
    final approvalId = Get.parameters['approvalId'];
    if (approvalId != null) {
      fetchApprovalDetail(approvalId);
    } else {
      print('No approvalId found in parameters.');
    }
  }
  ...
  Future<void> fetchApprovalDetail(String approvalId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        Get.snackbar('Error', 'Failed to fetch approval detail',
            snackPosition: SnackPosition.TOP);
      }
    } catch (error) {
      print('Error fetching data: $error');
      Get.snackbar('Error', 'Error fetching approval detail: $error',
          snackPosition: SnackPosition.TOP);
    }
  }
}
